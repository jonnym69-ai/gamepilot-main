...
export const SteamImportModal: React.FC<SteamImportModalProps> = ({ isOpen, onClose, onImportGames }) => {
  ...
  const { actions, isLoading } = useLibraryStore()
  const { user } = useAuth()
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                disabled={isLoading || !!user?.steamId}
              />
              <p className="text-xs text-gray-400 mt-1">
                {user?.steamId ? 'Steam ID auto-filled from your login' : 'Find your Steam ID in your profile URL'}
              </p>
            </div>

...
export function refreshGameGenres() {
  ...
  const updatedGames = games.map(game => {
    ...
    if (game.id.startsWith('steam-') && game.genres.length === 1 && 
      ...
      try {
        ...
        console.log(`   Old genres: [${game.genres.map((g: any) => typeof g === 'string' ? g : g.name).join(', ')}]`)
        console.log(`   New genres: [${updatedGame.genres.map((g: any) => g.name).join(', ')}]`)
  ...
  const validGames = updatedGames.filter(game => 
    game.genres.every(genre => 
      typeof genre === 'object' && 
      'id' in genre && 
      'name' in genre && 

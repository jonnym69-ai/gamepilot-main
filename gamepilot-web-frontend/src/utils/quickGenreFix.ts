  ...
  const updatedGames = games.map(game => {
    ...
    const hasUnknownGenre = game.genres.length === 1 && 
      (typeof game.genres[0] === 'string' && game.genres[0] === 'unknown' ||
       typeof game.genres[0] === 'object' && (game.genres[0] as any).name === 'unknown')
    ...
    if (hasUnknownGenre) {
      ...
      if (title.includes('rpg') || title.includes('final fantasy') || title.includes('witcher') || 
        ...
        const rpgGenre = GENRES.find(g => g.name.toLowerCase() === 'rpg')
        if (rpgGenre) {
          newGenres.push({
            id: rpgGenre.id,
            name: rpgGenre.name,
      ...
      if (title.includes('shooter') || title.includes('fps') || title.includes('call of duty') || 
        ...
        const shooterGenre = GENRES.find(g => g.name.toLowerCase() === 'shooter')
        if (shooterGenre) {
          newGenres.push({
            id: shooterGenre.id,
      ...
      if (title.includes('strategy') || title.includes('rts') || title.includes('civilization') || 
        ...
        const strategyGenre = GENRES.find(g => g.name.toLowerCase() === 'strategy')
        if (strategyGenre) {
          newGenres.push({
            id: strategyGenre.id,
      ...
      if (title.includes('adventure') || title.includes('zelda') || title.includes('tomb raider') || 
        ...
        const adventureGenre = GENRES.find(g => g.name.toLowerCase() === 'adventure')
        if (adventureGenre) {
          newGenres.push({
      ...
      if (title.includes('racing') || title.includes('need for speed') || title.includes('gran turismo') || 
        ...
        const racingGenre = GENRES.find(g => g.name.toLowerCase() === 'racing')
        if (racingGenre) {
          newGenres.push({
            id: racingGenre.id,
      ...
      if (title.includes('sports') || title.includes('fifa') || title.includes('nba') || 
        ...
        const sportsGenre = GENRES.find(g => g.name.toLowerCase() === 'sports')
        if (sportsGenre) {
          newGenres.push({
            id: sportsGenre.id,

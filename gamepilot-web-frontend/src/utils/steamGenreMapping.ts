...
const STEAM_GENRE_MAPPING: Record<string, string> = {
  'Action': 'action',
  'Adventure': 'adventure', 
  'RPG': 'rpg',
  'Strategy': 'strategy',
...
const STEAM_TAG_TO_MOOD_MAPPING: Record<string, string[]> = {
  // Competitive tags
...
const STEAM_CATEGORY_TO_MOOD_MAPPING: Record<string, string[]> = {
...
export const mapGenreToMoods = (genreId: string): string[] => {
  return GENRE_TO_MOOD_MAPPING[genreId] || ['chill']
}

// Get mood for a game based on comprehensive metadata analysis
export const getGameMoods = (genreIds: string[], steamTags?: string[], steamCategories?: string[], playtime?: number): string[] => {
  const allMoods: string[] = []
  
  // 1. Get moods from genres (primary source) - use core mapping
  genreIds.forEach(genreId => {
    const genreMoods = GENRE_TO_MOOD_MAPPING[genreId] || []
...
export const processSteamGameData = (steamGame: any, steamDetails?: any) => {
  // Map Steam genres to our genres
  const steamGenres = steamDetails?.genres?.map((g: any) => g.description) || []
  const mappedGenres = steamGenres.map(mapSteamGenre).filter((g: string) => g !== 'unknown')
  ...
  const steamTags = steamDetails?.tags?.map((t: any) => t.description) || []
  const steamCategories = steamDetails?.categories?.map((c: any) => c.description) || []
  const playtime = steamGame.playtime_forever
  
  // Get comprehensive moods based on all metadata
  const moods = getGameMoods(finalGenres, steamTags, steamCategories, playtime)
  ...
  return {
    ...
    genres: finalGenres.map((genreId: string) => {
    ...
    steamData: {
      ...
      categories: steamDetails?.categories?.map((c: any) => c.description) || [],

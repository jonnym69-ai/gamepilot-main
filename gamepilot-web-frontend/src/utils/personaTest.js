...
class PersonaEngineTester {
    ...
    async testPersonaMetrics() {
        console.log('\n📊 Test 1: Persona Metrics');
        console.log('-'.repeat(30));
        try {
            const startTime = Date.now();
            const metrics = await personaEngineService_1.personaEngineService.getPersonaMetrics();
    ...
    async testGameLibraryAnalysis() {
        console.log('\n🎮 Test 2: Game Library Analysis');
        console.log('-'.repeat(30));
        try {
            const { games } = useLibraryStore_1.useLibraryStore.getState();
            ...
            games.forEach(game => {
                ...
                if (playtime < 5)
                    playtimeDistribution['< 5h'] = (playtimeDistribution['< 5h'] || 0) + 1;
                else if (playtime < 20)
                    playtimeDistribution['5-20h'] = (playtimeDistribution['5-20h'] || 0) + 1;
                else if (playtime < 50)
                    playtimeDistribution['20-50h'] = (playtimeDistribution['20-50h'] || 0) + 1;
                else
                    playtimeDistribution['> 50h'] = (playtimeDistribution['> 50h'] || 0) + 1;
                // Status analysis
    ...
    async testMoodDetection() {
        ...
        try {
            ...
            for (const mood of testMoods) {
                const suggestions = await personaEngineService_1.personaEngineService.getMoodBasedSuggestions(mood, 3);
                moodResults[mood] = {
    ...
    async testPlaystyleAnalysis() {
        console.log('\n🎯 Test 4: Playstyle Analysis');
        console.log('-'.repeat(30));
        try {
            const startTime = Date.now();
            const analysis = await personaEngineService_1.personaEngineService.getPlayPatternAnalysis();
            const endTime = Date.now();
    ...
    async testRecommendationEngine() {
        ...
        try {
            const startTime = Date.now();
            const recommendations = await personaEngineService_1.personaEngineService.getPersonaRecommendations(5);
            const endTime = Date.now();
            ...
            if (games && games.length > 0) {
                ...
                if (gamesWithAchievements > 0) {
                    const totalAchievements = games.reduce((sum, g) => {
                        if (g.achievements)
                            return sum + (g.achievements.unlocked || 0);
                        return sum;
                    }, 0);
                    const totalPossible = games.reduce((sum, g) => {
                        if (g.achievements)
                            return sum + (g.achievements.total || 0);
                        return sum;
                    }, 0);
    ...
    async testPerformance() {
        ...
        try {
            ...
            await personaEngineService_1.personaEngineService.getPersonaMetrics();
...
exports.PersonaEngineTester = PersonaEngineTester;
// Export for easy usage
exports.personaTester = new PersonaEngineTester();

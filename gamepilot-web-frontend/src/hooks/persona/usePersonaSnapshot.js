 ...
 */
function deriveGlobalPlayerSignals(games) {
    ...
    const recentlyPlayedGames = games.filter(game => game.lastPlayed &&
    ...
    const multiplayerGames = games.filter(game => {
        ...
        const hasMultiplayerTag = tags.some((tag) => tag.includes('multiplayer') ||
            tag.includes('co-op') ||
            tag.includes('pvp') ||
            tag.includes('online'));
        const hasSteamPlatform = game.platforms?.some((p) => p.code === 'steam');

...
const useWhatToBuy = () => {
    ...
    const wishlist = (0, wishlistStore_1.useWishlistStore)(state => state.wishlist);
    // Create stable random indices to prevent re-renders
    const randomGameIndex = (0, react_1.useMemo)(() => Math.floor(Math.random() * POPULAR_GAMES.length), []);
    const fallbackGameIndex = (0, react_1.useMemo)(() => Math.floor(Math.random() * POPULAR_GAMES.length), []);
    // Memoize all computation to prevent re-renders
    const computedRecommendation = (0, react_1.useMemo)(() => {
        // Get user's top moods from their library
        const userMoods = {};
        games.forEach(game => {
            if (game.genres) {
                const genreNames = game.genres.map(genre => typeof genre === 'string' ? genre : genre.name);
                const gameMoods = (0, moodTags_1.getMoodsForGenres)(genreNames);
        ...
        const scoredGames = POPULAR_GAMES.map(game => {
            ...
            // Mood matching bonus
            const gameMoods = (0, moodTags_1.getMoodsForGenres)(game.genres);
        ...
        const gameMoods = (0, moodTags_1.getMoodsForGenres)(topGame.genres);
...
exports.useWhatToBuy = useWhatToBuy;

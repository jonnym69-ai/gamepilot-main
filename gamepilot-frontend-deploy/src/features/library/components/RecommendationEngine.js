...
const RecommendationEngine = ({ games, userGames, currentMood, onRecommendationSelect, className = '' }) => {
    ...
    const analyzeUserPatterns = (0, react_1.useCallback)(() => {
        ...
        const recentGames = userGames
            ...
            .slice(0, 10);
        const lastPlayedGenres = recentGames.flatMap(game => game.genres?.map(genre => typeof genre === 'string' ? genre : genre.name || 'Unknown') || []).filter((genre, index, arr) => arr.indexOf(genre) === index);
        return {
            avgSessionLength,
            preferredGenres,
            moodPreferences,
    ...
    const getMoodRecommendations = (0, react_1.useCallback)((mood, _userPattern) => {
    ...
    const getSimilarGamesRecommendations = (0, react_1.useCallback)((_userPattern) => {

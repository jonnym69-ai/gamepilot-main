...
const testFiltering = () => {
    ...
    if (games.length > 0) {
        ...
        const socialGames = games.filter(game => {
            ...
            const hasMultiplayer = moods.some(mood => mood.toLowerCase().includes('multiplayer') ||
                mood.toLowerCase().includes('coop') ||
                mood.toLowerCase().includes('online')) || genres.some(genre => genre.includes('multiplayer') || genre.includes('online')) || title.includes('multiplayer');

...
export type MoodId = 'social' | 'competitive' | 'story' | 'creative' | 'chill' | 'focused' | 'energetic' | 'exploratory'

/**
 * Derive primary mood from game data using priority rules
 * First match wins - this encodes our mood philosophy
 */
export function deriveMoodFromGame(game: Game): MoodId | null {
  // Get game data
 ...
 */
export function autoAssignFeatures(game: Game): Game {
  const title = game.title.toLowerCase()
  const features: string[] = [...(game.moods || [])]

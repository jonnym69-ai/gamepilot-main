...
exports.validateMoodIntegration = validateMoodIntegration;
exports.generateValidationReport = generateValidationReport;
exports.runMoodValidation = runMoodValidation;
const moodFilterSystem_1 = require("./moodFilterSystem");
/**
 * Validate mood filter system with real game library
 */
function validateMoodIntegration(games) {
    console.log(`🧪 Validating mood filter system with ${games.length} games`);
    const results = [];
    // Test each mood filter
    for (const moodFilter of moodFilterSystem_1.MOOD_FILTERS) {
        console.log(`\n📊 Testing ${moodFilter.name} mood...`);
        // Get filtered games
        const filteredGames = (0, moodFilterSystem_1.filterGamesByMood)(games, moodFilter.id, 100);
        // Calculate overlap (games that match multiple moods)
        let overlapCount = 0;
        const gameMoodMatches = new Map();
        games.forEach(game => {
            const matchingMoods = [];
            moodFilterSystem_1.MOOD_FILTERS.forEach(filter => {
    ...
    const systemValidation = (0, moodFilterSystem_1.validateMoodSystem)(games);
    console.log(`\n🎯 System Validation:`);
 ...
 */
function analyzeMoodAccuracy(games, moodId) {
    const accuracy = {
        socialGames: 0,
        storyGames: 0,
        adventureGames: 0,
        ...
        switch (moodId) {
            case 'social':
                if (hasMultiplayerFeatures(game))
                    accuracy.socialGames++;
                break;
            case 'competitive':
                if (hasCompetitiveFeatures(game))
                    accuracy.competitiveGames++;
                break;
            case 'story':
                if (hasStoryFeatures(game))
                    accuracy.storyGames++;
                break;
            case 'adventure':
                if (hasAdventureFeatures(game))
                    accuracy.adventureGames++;
                break;
            case 'chill':
                if (hasRelaxingFeatures(game))
                    accuracy.chillGames++;
                break;
            case 'creative':
                if (hasCreativeFeatures(game))
                    accuracy.creativeGames++;
                break;
        }
    });
    return accuracy;
 ...
 */
function identifyMoodIssues(games, moodFilter) {
    const issues = [];
    if (games.length === 0) {
...
function hasMultiplayerFeatures(game) {
    const features = [
...
function hasCompetitiveFeatures(game) {
...
function hasStoryFeatures(game) {
    const storyFeatures = [
...
function hasAdventureFeatures(game) {
...
function hasRelaxingFeatures(game) {
...
function hasCreativeFeatures(game) {
 ...
 */
function runMoodValidation(games) {

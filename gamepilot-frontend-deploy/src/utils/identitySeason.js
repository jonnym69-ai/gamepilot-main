...
exports.getSeasonStats = exports.clearSeasonReports = exports.generateAllSeasonReports = exports.shouldGenerateSeasonReport = exports.getSeasonReports = exports.saveSeasonReport = exports.generateSeasonReport = void 0;
/**
 * Generate a season report from snapshots and milestones
 */
const generateSeasonReport = (snapshots, allMilestones, options = {}) => {
    ...
    const existingReports = (0, exports.getSeasonReports)(options);
 ...
 */
const saveSeasonReport = (report, options = {}) => {
    const { maxSeasons = 12, storageKey = 'identity_seasons' } = options;
    try {
...
exports.saveSeasonReport = saveSeasonReport;
/**
 * Get all season reports from localStorage
 */
const getSeasonReports = (options = {}) => {
...
exports.getSeasonReports = getSeasonReports;
/**
 * Check if a new season report should be generated
 */
const shouldGenerateSeasonReport = (lastReport) => {
    if (!lastReport)
        return true;
    const now = new Date();
...
exports.shouldGenerateSeasonReport = shouldGenerateSeasonReport;
/**
 * Generate season reports for all available months
 */
const generateAllSeasonReports = (snapshots, allMilestones, options = {}) => {
    const reports = [];
    ...
    Array.from(snapshotsByMonth.entries())
        ...
        const report = (0, exports.generateSeasonReport)(monthSnapshots, allMilestones, options);
        if (report) {
            reports.push(report);
        }
    });
...
exports.generateAllSeasonReports = generateAllSeasonReports;
/**
 * Calculate dominant moods for the month
 */
const calculateDominantMoods = (snapshots) => {
    const moodCounts = {};
    snapshots.forEach(snapshot => {
 ...
 */
const calculateSessionLengthTrends = (snapshots) => {
    const trends = { short: 0, medium: 0, long: 0 };
    snapshots.forEach(snapshot => {
        trends[snapshot.preferredSessionLength]++;
    });
    return trends;
};
/**
 ...
 */
const calculateTimeOfDayPatterns = (snapshots) => {
 ...
 */
const getNewMilestonesForMonth = (allMilestones, monthSnapshots, existingReports) => {
    if (monthSnapshots.length === 0)
        return [];
 ...
 */
const calculateTopIdentityGames = (snapshots) => {
    const gameCounts = {};
    snapshots.forEach(snapshot => {
 ...
 */
const calculateCompletionRateChange = (previousMonthSnapshots, currentMonthSnapshots) => {
 ...
 */
const calculatePlayPatternShifts = (previousMonthSnapshots, currentMonthSnapshots) => {
 ...
 */
const getPreviousMonthSnapshots = (allSnapshots, currentMonth, currentYear, existingReports) => {
...
const generateSeasonNarrative = (data) => {
    const { dominantMoods, sessionLengthTrends, timeOfDayPatterns, newMilestones, topIdentityGames, snapshotCount, completionRateChange, playPatternShifts, monthName, year } = data;
...
const generateFullSeasonNarrative = (data) => {
    ...
    const totalSessions = Object.values(sessionLengthTrends).reduce((a, b) => a + b, 0);
    const dominantSession = Object.entries(sessionLengthTrends)
        .sort(([, a], [, b]) => b - a)[0]?.[0] || 'medium';
    const sessionPercentage = ((sessionLengthTrends[dominantSession] || 0) / totalSessions) * 100;
 ...
 */
const clearSeasonReports = (options = {}) => {
    const { storageKey = 'identity_seasons' } = options;
    try {
...
exports.clearSeasonReports = clearSeasonReports;
/**
 * Get season statistics
 */
const getSeasonStats = (options = {}) => {
    const reports = (0, exports.getSeasonReports)(options);
    if (reports.length === 0) {
        return {
            totalSeasons: 0,
            totalSnapshots: 0,
